...
class Dataset(object):
    def __init__(self,random_walk_txt,node_type_mapping_txt,window_size):
        index2token,token2index,word_and_counts,index2frequency,node_context_pairs= self.parse_random_walk_txt(random_walk_txt,window_size)  # 构建语料
        self.window_size = window_size
        self.nodeid2index = token2index
        self.index2nodeid = index2token
        self.index2frequency = index2frequency
        index2type,type2indices = self.parse_node_type_mapping_txt(node_type_mapping_txt, self.nodeid2index)  # 构建点类别关系
        self.index2type = index2type  # 点边关系
        self.type2indices = type2indices  # 类型边连接到的节点
        self.node_context_pairs= node_context_pairs  # window选择的点，训练语料
        self.prepare_sampling_dist(index2frequency,index2type,type2indices)  # 采样函数
        self.shffule()
        self.count = 0
        self.epoch = 1
        ...
        with open(node_type_mapping_txt) as f:
            for line in f:
                ...
                if len(pair[0])==0 or len(pair[1])==0:
                    ...
                    print(pair)
                    continue
                index2type[nodeid2index[pair[0]]]=pair[1]  # pari[0]:pair[1]; edge关系; 点所属于的类别

        type2indices = {}
        all_types = set(index2type.values())  # 所有的类型
        for node_type in all_types:
            type2indices[node_type]=[]  # 每个类型建立一个list
        # 建立type相连接的点
        for node_index,node_type in index2type.items():
            type2indices[node_type].append(node_index)  # 类型节点所连接到的边

        # make array because it will be used with numpy later
        for node_type in all_types:  # 所有类型
            type2indices[node_type]=np.array(type2indices[node_type])  # 将list转换成array格式
        # 节点对应到的类型边, 类型边对应到array格式
    ...
    def parse_random_walk_txt(self,random_walk_txt,window_size):
        ...
        with open(random_walk_txt) as f:
            for line in f:
                ...
                for word in sent:       
                    if len(word) == 0:
                        continue
                    if word in word_and_counts:  # word_and_counts.has_key(word) python3.X 版本要改
        ...
        index2token = dict((i, word) for i, word in enumerate(word_and_counts.keys()) )  # index:word
        token2index = dict((v, k) for k, v in index2token.items())   # word:index
        index2frequency = dict((token2index[word],freq) for word,freq in word_and_counts.items() )  # 索引对应到的频率的值

        #word_word=scipy.sparse.lil_matrix((len(token2index), len(token2index)), dtype=np.int32)
        node_context_pairs = []  # let's use naive way now; 训练set，构建语料

        print("window size %d"%window_size)
        # 语义的部分
        with open(random_walk_txt) as f:
            for line in f:
                sent = [token2index[word.strip()] for word in line.split(' ') if word.strip() in token2index]  # 词对应到的index
                sent_length=len(sent)
                for target_word_position,target_word_idx in enumerate(sent):  # index, word
                    start=max(0,target_word_position-window_size)   # window_size的区间
                    end=min(sent_length,target_word_position+window_size)  # window_size的end区间《源码: window_size+1》
        ...
        # index:token; token:index; word:cnt; index:frequcency; node_context_pairs-上下文
    ...
    def get_negative_samples(self,pos_index,num_negatives,care_type):
        ...
        if not care_type:  # metapath2vec
            ...
            node_type = self.index2type[pos_index]  # 上下文对应到的节点类型
            sampling_probs = self.type2probs[node_type]  # 上下文对应到的采样概率
            sampling_candidates = self.type2indices[node_type]  # 上下文连接到的节点
    ...
    def prepare_sampling_dist(self,index2frequency,index2type,type2indices):
        sampling_prob = np.zeros(len(index2frequency))  # 节点频率; 所有节点矩阵
        for i in range(len(index2frequency)):
            sampling_prob[i]=index2frequency[i]  # 节点频率
        sampling_prob = sampling_prob**(3.0/4.0)  # 3/4次方，采样概率; #from http://mccormickml.com/2017/01/11/word2vec-tutorial-part-2-negative-sampling/

        #normalize the distributions
        #for caring type，考虑到类型的sample情况
        all_types = set(index2type.values())  # 所有的类型
        type2probs = {}
        for node_type in all_types:
            indicies_for_a_type = type2indices[node_type]   # type相连接的节点
            type2probs[node_type] = np.array(sampling_prob[indicies_for_a_type])  # 节点的采样概率
            type2probs[node_type] = type2probs[node_type]/np.sum(type2probs[node_type])  # 概率归一化


        #if not careing type
        sampling_prob = sampling_prob/np.sum(sampling_prob)  # 均匀采样的概率

        self.sampling_prob = sampling_prob  # 均匀采样
        self.type2probs = type2probs  # 类别概率采样
