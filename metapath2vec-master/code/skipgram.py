...
def build_model(BATCH_SIZE,VOCAB_SIZE,EMBED_SIZE,NUM_SAMPLED):
    ...
    with tf.name_scope('embedding_matrix'):
        embed_matrix = tf.Variable(tf.random_uniform([VOCAB_SIZE, EMBED_SIZE], -1.0, 1.0),   # word embedding 变量
                            name='embed_matrix')

    # define the inference
    with tf.name_scope('loss'):
        embed = tf.nn.embedding_lookup(embed_matrix, center_node, name='embed')  # 生成embedding; 按照ids顺序返回params中的第ids行

        # construct variables for NCE loss; weights: A `Tensor` of shape `[num_classes, dim]`
...
def train(center_node_placeholder,context_node_placeholder,negative_samples_placeholder,loss,dataset,optimizer,NUM_EPOCHS,BATCH_SIZE,NUM_SAMPLED,care_type,LOG_DIRECTORY,LOG_INTERVAL,MAX_KEEP_MODEL):
    ...
    *epoch index starts from 1! not 0.
    '''
    care_type = True if care_type==1 else False  # metapath2vec or metapath2vec++ 采样方式
    ...
    with tf.Session() as sess:
        ...
        while (dataset.epoch <= NUM_EPOCHS):
            ...
            center_node_batch,context_node_batch = dataset.get_batch(BATCH_SIZE)  # 获取训练个batch语料
