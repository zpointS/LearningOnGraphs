...
def load_feature_construct_H(data_dir,
    ...
    """
    # init feature
    if use_mvcnn_feature or use_mvcnn_feature_for_structure:  # MVCNN特征
        mvcnn_ft, lbls, idx_train, idx_test = load_ft(data_dir, feature_name='MVCNN')  # (12311, 4096)
    if use_gvcnn_feature or use_gvcnn_feature_for_structure:  # GVCNN特征
        gvcnn_ft, lbls, idx_train, idx_test = load_ft(data_dir, feature_name='GVCNN')  # (12311, 2048)
    if 'mvcnn_ft' not in dir() and 'gvcnn_ft' not in dir():
        raise Exception('None feature initialized')

    # construct feature matrix; 构造特征
    fts = None
    if use_mvcnn_feature:
        fts = hgut.feature_concat(fts, mvcnn_ft)
    if use_gvcnn_feature:
        fts = hgut.feature_concat(fts, gvcnn_ft)  # fts, GVCNN特征
    if fts is None:
    ...
    H = None
    if use_mvcnn_feature_for_structure:  # MVCNN特征构建超边
        ...
        H = hgut.hyperedge_concat(H, tmp)  # 超边concat;
    if use_gvcnn_feature_for_structure:  # GVCNN特征构建超边
        tmp = hgut.construct_H_with_KNN(gvcnn_ft, K_neigs=K_neigs,
                                        split_diff_scale=split_diff_scale,
                                        is_probH=is_probH, m_prob=m_prob)  # (12311, 12311)
        H = hgut.hyperedge_concat(H, tmp)  # (12311, 24622); (H>0).sum(axis=0)超边，连接10个节点
    if H is None:
