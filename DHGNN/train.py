...
def train_test_model(cfg):
    ...
    H = _edge_dict_to_H(edge_dict)  # 构造超边矩阵
    G = _generate_G_from_H(H)  # 超图更新矩阵

    G = torch.Tensor(G).to(device)

    fts = torch.Tensor(fts).to(device)  # 节点特征
    lbls = torch.Tensor(lbls).squeeze().long().to(device)  # label
...
if __name__ == '__main__':
    ...
    setup_seed(seed_num)  # 设定随机数
    print('Using random seed: ', seed_num)

    cfg = get_config('config/config.yaml')  # 导入config文件
